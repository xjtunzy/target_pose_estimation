...
def jude_head_end_of_arcs(e1h,e1e,e2h,e2e):
    ...
    if dis4<5:j4 = 1
    return j1+j2+j3+j4
def fit_ellipse_b2ac(points):
    # 输入检查：转换为Nx2的浮点数组
    pts = np.array(points, dtype=np.float32)
    n = pts.shape[0]
    assert n >= 5, "至少需要5个点来拟合椭圆"
    
    # 初始化变量
    c = np.mean(pts, axis=0)  # 点集中心
    scale = 100.0 / (np.sum(np.abs(pts - c))) if np.sum(np.abs(pts - c)) > 1e-6 else 100.0
    
    # 第一次拟合：求解一般椭圆方程系数 (A-E)
    A = np.zeros((n, 5), dtype=np.float64)
    b = np.full((n, 1), 10000.0, dtype=np.float64)
    
    scaled_pts = (pts - c) * scale
    A[:, 0] = -scaled_pts[:, 0] ** 2  # A
    A[:, 1] = -scaled_pts[:, 1] ** 2  # B
    A[:, 2] = -scaled_pts[:, 0] * scaled_pts[:, 1]  # C
    A[:, 3] = scaled_pts[:, 0]  # D
    A[:, 4] = scaled_pts[:, 1]  # E
    ...
    rhs = np.array([gfp[3], gfp[4]], dtype=np.float64)
    center_offset = np.linalg.solve(M, rhs)  # 解线性方程组
    
    # 第二次拟合：固定中心后重新求解A-C
    A2 = np.zeros((n, 3), dtype=np.float64)
    b2 = np.ones((n, 1), dtype=np.float64)
    centered_pts = scaled_pts - center_offset
    A2[:, 0] = centered_pts[:, 0] ** 2  # A
    A2[:, 1] = centered_pts[:, 1] ** 2  # B
    A2[:, 2] = centered_pts[:, 0] * centered_pts[:, 1]  # C
    
    gfp_new = np.linalg.lstsq(A2, b2, rcond=None)[0].flatten()
    
    # 计算旋转角度和轴长
    angle_rad = -0.5 * np.arctan2(gfp_new[2], gfp_new[1] - gfp_new[0])
    t = gfp_new[2] / np.sin(-2 * angle_rad) if np.abs(gfp_new[2]) > 1e-8 else gfp_new[1] - gfp_new[0]
    
    axis1 = 2.0 / (gfp_new[0] + gfp_new[1] - t) if np.abs(gfp_new[0] + gfp_new[1] - t) > 1e-8 else 0
    axis2 = 2.0 / (gfp_new[0] + gfp_new[1] + t) if np.abs(gfp_new[0] + gfp_new[1] + t) > 1e-8 else 0
    
    width = 2 * np.sqrt(axis1) / scale if axis1 > 0 else 0
    height = 2 * np.sqrt(axis2) / scale if axis2 > 0 else 0
    ...
    angle_deg = angle_deg % 360  # 归一化到[0, 360)
    
    # 返回RotatedRect格式 (中心需加回偏移)
    center = center_offset / scale + c
    return (center[0], center[1]), (width, height), angle_deg
...
if __name__ == "__main__":
    ...
    filename2 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im3949.jpg"
    filename3 = r"E:\dataset\aamed_ellipse_datasets\Prasad Images - Dataset Prasad\images\079_0033.jpg"
    filename4 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im9882.jpg"
    filename5 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im8237.jpg"
    filename6 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im9185.jpg"
    filename7 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im100628.jpg"
    filename8 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im104601.jpg"
    filename9 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\im119755.jpg"
    filename10 = r"E:\dataset\aamed_ellipse_datasets\Random Images - Dataset #1\images\01295.jpg"
    filename11 = r"E:\dataset\aamed_ellipse_datasets\Prasad Images - Dataset Prasad\images\193_0094.jpg"
    filename12 = r"E:\dataset\aamed_ellipse_datasets\Prasad Images - Dataset Prasad\images\031_0012.jpg"
    filename13 = r"E:\dataset\aamed_ellipse_datasets\Prasad Images - Dataset Prasad\images\002_0038.jpg"
    filename14 = r"E:\dataset\aamed_ellipse_datasets\Prasad Images - Dataset Prasad\images\169_0015.jpg"
    img = cv.imread(filename14)
    gray = cv.cvtColor(img,cv.COLOR_BGR2GRAY)
    gray = cv.GaussianBlur(gray, (5, 5), 1.0)
    # medianBlur_ksize = 3
    ...
    canvas8 = 255*np.ones_like(img)
    canny,_ = EC.mark_edge_canny(gray)
    #canny,_,_ = EC.mark_edge_canny(gray)
    # 3. 将边缘位置设为灰色（128,128,128）
    edge_mask = (canny == 255)  # 找到边缘像素位置
    canvas7[edge_mask] = [128, 128, 128]  # BGR格式的灰色
    canvas8[edge_mask] = [128, 128, 128]  # BGR格式的灰色
    #canny,_ = EC.mark_edge_canny(gray)
    #contours = edge_detect.detect_edges(canny)
    # draw.draw_arcs(canvas1,contours)
    # cv.imwrite(f"img_of_process\\arcs.bmp",canvas1)
    ...
    for e in arcs2:
        ...
        t = 1-dis_h_e/(dis_h_m+dis_e_m)
        print(f"t: {t}")
        _t = 0.02  #原文中给出的范围为0.01-0.04
        if t<_t:continue
        arcs3.append(e)
        oritend = cv.minAreaRect(np.array(e))
        per_h_w = max(oritend[0][0],oritend[0][1])/min(oritend[0][0],oritend[0][1])
        print(f"per_h_w: {per_h_w}")
        # if per_h_w>4:continue
        # arcs3.append(e)
    draw.draw_arcs(canvas4,arcs3)
    #cv.waitKey(0)
    # for i in range(len(arcs2)):
        ...
        for a_j in arcs3:
            ...
            for a_k in G:
                ...
                if inter1== None or inter2 == None:
                    ...
                    if 1 and jude_head_end_of_arcs(a_j_h,a_j_e,a_k_h,a_k_e):
                        num +=1
                    else:
                        inter3 = get_intersection_point2(a_j_h,a_j_e,a_k_h,a_k_e)
                        offset = 1
                        ...
                        vk = a_k_h[0] - a_k_e[0]
                        wk = (a_k_h[1] - a_k_e[1])*a_k_h[0]-(a_k_h[0] - a_k_e[0])*a_k_h[1]
                        es = cv.fitEllipse(np.array(a_j+a_k))
                        elltt = param_diversion_ell2ell(es)
                        # print(f"es: {es}")
                        # cv.ellipse(canvas4,(int(elltt[0]),int(elltt[1])),(int(elltt[2]),int(elltt[3])),elltt[4]/math.pi*180,0,360,(255,100,0),1)
                        # cv.imshow("1",canvas4)
                        # cv.waitKey(0)
                        a,b,c,d,e,f = param_diversion(es)
                        if es==None:
                            consistent = False
                            break
                        #交比
    ...
    for g in seto:
        ...
        for e in g:
            e_all +=e
        #ell = fit_ellipse_b2ac(np.array(e_all))
        ell = cv.fitEllipse(np.array(e_all))
        if ell==None:
            ell_ans.append(ell)
            continue
        #需要去验证椭圆是否具有较高的置信度
        ...
        P = []
        num = 0
        ell = param_diversion_ell2ell(ell)
        x_c,y_c,A,B,_ = ell
        if A==0 or B==0 :
            ell_ans.append(None)
            continue
        ...
        print(f"内点率2: {R_c2}")
        print(f"评价: {R_jude}")
        ell_jude.append(R_jude)
        #if R_c<0.4:ell=None
        if R_jude<0.7:ell=None
        #计算张角
        label_len = 1
        if len(g)>1:label_len =0
        an_A = 0
        ...
        print(f"label: {label_len} an_A: {an_A}")
        ell_ans.append(ell)
    cv.imwrite("img_of_process\\curved_arc_orig.bmp",canvas4)
    for i,ell in enumerate(ell_ans):
        #print(f"ell: {ell}")
        if ell ==None:continue
        cv.ellipse(canvas4,(int(ell[0]),int(ell[1])),(int(ell[2]),int(ell[3])),ell[4]/math.pi*180,0,360,(255,100,0),2)
    ...
    while ell_ans:
        ...
        for ee in to_remove:
            ell_ans.remove(ee)
    print(f"聚类之后的数量：{len(ell_after_cluster)}")
    for i,ell in enumerate(ell_after_cluster):
        #print(f"ell: {ell}")
        if ell ==None:continue
        cv.ellipse(canvas8,(int(ell[0]),int(ell[1])),(int(ell[2]),int(ell[3])),ell[4]/math.pi*180,0,360,(0,255,0),2)
        cv.ellipse(img,(int(ell[0]),int(ell[1])),(int(ell[2]),int(ell[3])),ell[4]/math.pi*180,0,360,(0,255,0),2)
    cv.imshow("ans",img)
    cv.imwrite("img_of_process\\curved_arc_and_ells.bmp",canvas7)
    cv.imwrite("img_of_process\\ell_ans.bmp",canvas8)
    cv.waitKey(0)
    print(f"arcs2_spin: {arcs2_spin}")
